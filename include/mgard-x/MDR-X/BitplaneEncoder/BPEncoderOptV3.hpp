#ifndef _MDR_BP_ENCODER_OPT_V3_HPP
#define _MDR_BP_ENCODER_OPT_V3_HPP
...
template <typename T_data, typename T_fp, typename T_sfp, typename T_bitplane,
          ...
          typename DeviceType>
class BPEncoderOptV3Functor : public Functor<DeviceType> {
public:
  MGARDX_CONT
  ...
  MGARDX_EXEC void EncodeBinary() {
    ...
    for (SIZE batch_idx = batch_idx_start; batch_idx < num_batches;
      ...
      fp_data[data_idx] = (T_fp)fabs(shifted_data);
  ...
  MGARDX_CONT size_t shared_memory_size() {
    size_t size = sizeof(T_fp) * BATCH_SIZE * 8;
    size += sizeof(T_bitplane) * MAX_BITPLANES * 8; // for encoded bitplanes
    return size;
  }
...
template <typename T_data, typename T_fp, typename T_sfp, typename T_bitplane,
          typename T_error, bool NegaBinary, bool CollectError,
          typename DeviceType>
class BPEncoderOptV3Kernel : public Kernel {
public:
  ...
  using FunctorType =
      BPEncoderOptV3Functor<T_data, T_fp, T_sfp, T_bitplane, T_error,
...
class BPDecoderOptV3Functor : public Functor<DeviceType> {
public:
  MGARDX_CONT
...
public:
  ...
  using FunctorType = BPDecoderOptV3Functor<T_data, T_fp, T_sfp, T_bitplane,
...
public:
  ...
  BPEncoderOptV3() : initialized(false) {
  ...
  BPEncoderOptV3(Hierarchy<D, T_data, DeviceType> &hierarchy) {
    static_assert(std::is_floating_point<T_data>::value,
                  "GeneralBPEncoder: input data must be floating points.");
    static_assert(!std::is_same<T_data, long double>::value,
                  "GeneralBPEncoder: long double is not supported.");
  ...
  void encode(SIZE n, int num_bitplanes, int32_t exp,
    ...
    DeviceLauncher<DeviceType>::Execute(
        BPEncoderOptV3Kernel<T_data, T_fp, T_sfp, T_bitplane, T_error,
                             NegaBinary, CollectError, DeviceType>(
  ...
  void progressive_decode(SIZE n, SIZE starting_bitplanes, int num_bitplanes,
    ...
    if (num_bitplanes > 0) {
      DeviceLauncher<DeviceType>::Execute(
          BPDecoderOptV3Kernel<T_data, T_fp, T_sfp, T_bitplane, NegaBinary,
