#ifndef _MDR_BATCHED_BP_ENCODER_HPP
#define _MDR_BATCHED_BP_ENCODER_HPP
...
class GroupedEncoderFunctor2 : public Functor<DeviceType> {
public:
  MGARDX_CONT
  GroupedEncoderFunctor2() {}
  MGARDX_CONT
  GroupedEncoderFunctor2(
...
template <typename T, typename T_bitplane, typename T_error, OPTION BinaryType,
          ...
          typename DeviceType>
class GroupedEncoderKernel2 : public Kernel {
public:
  constexpr static bool EnableAutoTuning() { return false; }
  constexpr static std::string_view Name = "grouped bp encoder";
  MGARDX_CONT
  GroupedEncoderKernel2(SIZE n, SIZE num_batches_per_TB, SIZE num_bitplanes,
  ...
  using FunctorType =
      GroupedEncoderFunctor2<T, T_fp, T_sfp, T_bitplane, T_error, BinaryType,
...
class GroupedDecoderFunctor2 : public Functor<DeviceType> {
public:
  MGARDX_CONT
  GroupedDecoderFunctor2() {}
  MGARDX_CONT
  GroupedDecoderFunctor2(SIZE n, SIZE num_batches_per_TB,
                         SIZE starting_bitplane, SIZE num_bitplanes, SIZE exp,
                         SubArray<2, T_bitplane, DeviceType> encoded_bitplanes,
...
class GroupedDecoderKernel2 : public Kernel {
public:
  constexpr static bool EnableAutoTuning() { return false; }
  constexpr static std::string_view Name = "grouped bp decoder";
  MGARDX_CONT
  GroupedDecoderKernel2(SIZE n, SIZE num_batches_per_TB, SIZE starting_bitplane,
  ...
  using T_fp = typename std::conditional<std::is_same<T, double>::value,
                                         uint64_t, uint32_t>::type;
  using FunctorType =
      GroupedDecoderFunctor2<T, T_fp, T_sfp, T_bitplane, BinaryType,
...
template <DIM D, typename T_data, typename T_bitplane, typename T_error,
          typename DeviceType>
class BatchedBPEncoder
    : public concepts::BatchedBitplaneEncoderInterface<D, T_data, T_bitplane,
                                                       T_error, DeviceType> {
public:
  BatchedBPEncoder() : initialized(false) {
  ...
  BatchedBPEncoder(Hierarchy<D, T_data, DeviceType> &hierarchy) {
    static_assert(std::is_floating_point<T_data>::value,
                  "GeneralBPEncoder: input data must be floating points.");
    static_assert(!std::is_same<T_data, long double>::value,
                  "GeneralBPEncoder: long double is not supported.");
  ...
  encode(std::vector<SIZE> n, SIZE num_bitplanes, std::vector<int32_t> exp,
         ...
         std::vector<std::vector<SIZE>> &streams_sizes, int queue_idx) {
    ...
    for (int level_idx = 0; level_idx < hierarchy->l_target() + 1;
      ...
      DeviceLauncher<DeviceType>::Execute(
          GroupedEncoderKernel2<T_data, T_bitplane, T_error, BINARY_TYPE,
                                DATA_ENCODING_ALGORITHM,
                                ERROR_COLLECTING_ALGORITHM, DeviceType>(
              n[level_idx], num_batches_per_TB, num_bitplanes, exp[level_idx],
              v[level_idx], encoded_bitplanes[level_idx], level_errors_work),
          queue_idx);
      SIZE reduce_size = num_blocks(n[level_idx]);
      for (int i = 0; i < num_bitplanes + 1; i++) {
        SubArray<1, T_error, DeviceType> curr_errors({reduce_size},
                                                     level_errors_work(i, 0));
        SubArray<1, T_error, DeviceType> sum_error({1},
                                                   level_errors[level_idx](i));
        DeviceCollective<DeviceType>::Sum(reduce_size, curr_errors, sum_error,
                                          level_error_sum_work_array, true,
                                          queue_idx);
      }
      for (int i = 0; i < num_bitplanes; i++) {
        streams_sizes[level_idx][i] =
            buffer_size(n[level_idx]) * sizeof(T_bitplane);
      }
    }
  }
  ...
  decode(std::vector<SIZE> n, std::vector<uint8_t> num_bitplanes,
         ...
         std::vector<SubArray<2, T_bitplane, DeviceType>> encoded_bitplanes,
         std::vector<SubArray<1, T_data, DeviceType>> v, int queue_idx) {}

  // decode the data and record necessary information for progressiveness
  void progressive_decode(
    ...
    for (int level_idx = 0; level_idx < hierarchy->l_target() + 1;
         level_idx++) {
      if (num_bitplanes[level_idx] > 0) {
        DeviceLauncher<DeviceType>::Execute(
            GroupedDecoderKernel2<T_data, T_bitplane, BINARY_TYPE,
                                  DATA_DECODING_ALGORITHM, DeviceType>(
                n[level_idx], num_batches_per_TB, starting_bitplanes[level_idx],
                num_bitplanes[level_idx], exp[level_idx],
                encoded_bitplanes[level_idx], level_signs[level_idx],
                v[level_idx]),
            queue_idx);
      }
    }
