...
mgard_x_src_dir=.
# Build directory
build_dir=./build-cuda-blackwell
# Number of processors used for building
num_build_procs=$1
# Installtaion directory
install_dir=./install-cuda-blackwell
...
# NVCOMP is not built here: mgard-x's CUDA backend (MGARD_ENABLE_CUDA)
...
#build MGARD
# MDR is off: it pulls in find_package(MPI REQUIRED) and this node has no MPI installed.
...
cmake -S ${mgard_x_src_dir} -B ${mgard_x_build_dir} \
    -DCMAKE_PREFIX_PATH="${zstd_install_dir};${protobuf_install_dir};${catch2_install_dir}"\
    -DMGARD_ENABLE_CUDA=ON\
    -DMGARD_ENABLE_OPENMP=OFF\
    -DMGARD_ENABLE_MDR=OFF\
